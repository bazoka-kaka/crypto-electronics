...
const Create = () => {
  ...
  useEffect(() => {
    ...
    setValidConfirm(pwd === pwdConfirm);
  }, [pwd, pwdConfirm]);

  useEffect(() => {
    setErrMsg("");
  }, [user, pwd, pwdConfirm, fullname, email, roles]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosPrivate.post(
        "/users",
        JSON.stringify({ user, pwd, email, fullname, roles }),
        {
      ...
      if (!err?.response) {
        ...
        setErrMsg("Username is taken already.");
    ...
    <div>
      ...
      <div className="p-4 mt-4 bg-white rounded-md">
        <form>
          ...
          <label htmlFor="confirmation" className="block mt-4 font-semibold">
            Password Confirmation{" "}
            <FontAwesomeIcon
              icon={faCheck}
              className={
                validConfirm && pwdConfirm
                  ? "inline-block bg-green-400 w-4 h-4 text-white rounded-full"
                  : "hidden"
              }
            />
